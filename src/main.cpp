...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WTFCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WTFCoin Signed Message:\n";
...
{
    // WTFCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // old code
//    nSubsidy >>= (nHeight / 99999); // WTFCoin: 99,999

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // WTFCoin: 4 hours
static const int64 nTargetSpacing = 90; // WTFCoin: 90 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WTFCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 90; // WTFCoin: 90 seconds
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // WTFCoin: 1 day
...
{
    // WTFCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "9/9/14 NY times Ferguson Sets Broad Change For City Courts";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // WTFCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WTFCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WTFCoinMiner : ProcessBlock, block not accepted");
    }
